...
def fix(pik):
    pik.rectangle((int(sz / 2) - f + eccentricity, int(sz / 2) + f * 3 + eccentricity,
                   int(sz / 2) + f + eccentricity, int(sz / 2) - f * 3 + eccentricity),
                  fill=(0, 0, 255), outline=None)
    pik.rectangle((int(sz / 2) - f * 3 + eccentricity, int(sz / 2) + f + eccentricity,
...
for i in variation:
    ...
    draw.rectangle((int(sz / 2) - int(f / 2) + disparity*i, int(sz / 2) + int(ll / 2),
                    int(sz / 2) + int(f / 2) + disparity*i, int(sz / 2) - int(ll / 2)),
                   fill=(0, 0, 0), outline=None)

    draw.rectangle((int(sz / 2) + int(f / 2) + int(ll / 2), int(sz / 2) + int(f / 2),
                    int(sz / 2) - int(f / 2) - int(ll / 2), int(sz / 2) - int(f / 2)),
...
drawt.rectangle((int(sz / 2) + int(ll / 2), int(sz / 2) + int(f / 2),
                int(sz / 2) - int(ll / 2), int(sz / 2) - int(f / 2)),
                fill=(lb*2, 0, 0), outline=None)
fix(drawt)

basename = os.path.basename('cntl.png')
